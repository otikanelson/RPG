class CharacterManager {
    constructor() {
        ...
        this.characterStatsPanel = document.getElementById('characterStatsPanel');
        this.sidebarLevelRow = document.getElementById('sidebarLevelRow');
        this.sidebarHealthRow = document.getElementById('sidebarHealthRow');
        this.sidebarGoldRow = document.getElementById('sidebarGoldRow');
        this.sidebarLevelBarFill = document.getElementById('sidebarLevelBarFill');
        this.sidebarLevelValue = document.getElementById('sidebarLevelValue');
        ...
        this.characterAliases = {
            "Merchant Ragnor": "MerchantRagnor",
            "Shard Warden": "ShardWarden"
        };

        this.monsterBattleKeys = {
    ...
    getPrimaryCharacterKey(dialogue) {
        if (!dialogue?.characters?.length) return null;

        const nonPlayer = dialogue.characters.find((name) => name !== "Player");
        return this.resolveCharacterKey(nonPlayer || dialogue.characters[0]);
    }

    getMonsterDataForCharacter(characterKey) {
        const battleKey = this.monsterBattleKeys[characterKey];
        if (!battleKey) return null;

        const battleManager = window.dialogueManager?.battleManager;
        return battleManager?.monsters?.[battleKey] || null;
    }
    ...
    updateDisplay(dialogue) {
        ...
        const characterKey = this.getPrimaryCharacterKey(dialogue);

        if (characterKey) {
            const characterData = this.characters[characterKey];

            if (!characterData) {
                console.warn(`Character data not found for: ${characterKey}`);
                return;
            }
    ...
    hideSidebarStats() {
        if (this.characterStatsPanel) {
            this.characterStatsPanel.style.display = 'none';
        }
    }

    showSidebarStats() {
        if (this.characterStatsPanel) {
            this.characterStatsPanel.style.display = 'block';
        }
    }

    setStatRowVisibility(row, visible) {
        if (row) {
            row.style.display = visible ? 'flex' : 'none';
        }
    }

    updateSidebarStats(characterKey, characterData) {
        if (this.isShowingLocation) {
            this.hideSidebarStats();
            return;
        }

        this.showSidebarStats();

        if (characterData.isPlayer) {
            this.updatePlayerSidebarStats();
            return;
        }

        const monsterData = this.getMonsterDataForCharacter(characterKey);

        this.setStatRowVisibility(this.sidebarLevelRow, true);
        this.setStatRowVisibility(this.sidebarHealthRow, Boolean(monsterData));
        this.setStatRowVisibility(this.sidebarGoldRow, false);

        if (monsterData) {
            if (this.sidebarLevelBarFill) {
                this.sidebarLevelBarFill.style.width = `${Math.min(monsterData.level * 10, 100)}%`;
            }
            if (this.sidebarLevelValue) {
                this.sidebarLevelValue.textContent = monsterData.level.toString();
            }
            if (this.sidebarHealthBarFill) {
                this.sidebarHealthBarFill.style.width = '100%';
            }
            if (this.sidebarHealthValue) {
                this.sidebarHealthValue.textContent = monsterData.maxHealth.toString();
            }
            return;
        }

        if (this.sidebarLevelBarFill) {
            this.sidebarLevelBarFill.style.width = '100%';
        }
        if (this.sidebarLevelValue) {
            this.sidebarLevelValue.textContent = characterData.level;
        }
    }

    updatePlayerSidebarStats() {
        const gameLogic = window.gameLogic;
        if (!gameLogic?.stats) return;

        this.showSidebarStats();
        this.setStatRowVisibility(this.sidebarLevelRow, true);
        this.setStatRowVisibility(this.sidebarHealthRow, true);
        this.setStatRowVisibility(this.sidebarGoldRow, true);

        const { health, maxHealth, level, xp, gold } = gameLogic.stats;
        ...
        if (this.sidebarLevelBarFill) {
            this.sidebarLevelBarFill.style.width = `${levelPercent}%`;
        }
        if (this.sidebarLevelValue) {
            this.sidebarLevelValue.textContent = level.toString();
        }
        if (this.sidebarHealthBarFill) {
            this.sidebarHealthBarFill.style.width = `${healthPercent}%`;
        }
        if (this.sidebarHealthValue) {
            this.sidebarHealthValue.textContent = health.toString();
        }
        if (this.sidebarGoldValue) {
            this.sidebarGoldValue.textContent = gold.toString();
        }
    }
    ...
    refreshSidebarStats() {
        ...
        const characterData = this.characters[this.currentCharacterKey];
        if (!characterData) return;

        this.updateSidebarStats(this.currentCharacterKey, characterData);
    }
